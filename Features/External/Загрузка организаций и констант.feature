...
// Экспортный сценарий
@ExportScenarios
// Если будут одновременно загружены (открыты) экспортные и основные сценарии,
// то при запуске будет дублирование исполнения.
// Чтобы исключить - @IgnoreOnCIMainBuild (исключит сценарий из основной сборки)
@IgnoreOnCIMainBuild
...
//Для включения в библиотеку
@ТипШага: Загрузка данных
@Описание: Экпортный сценарий, который перезаполняет константу SSLIMAP 
